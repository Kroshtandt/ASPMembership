...
namespace ASPMembership2.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            {
                roles.Add(RolesManager.GetRoleById(RolesManager.GetRoleByName(role).Id, true));
            }

            foreach (var role in roles)
            {
                foreach (var method in role.Methods)
                {
                    if (!methods.Any(m => m.MethodId == method.MethodId))
                    {
                        methods.Add(method);
                    }
                }
